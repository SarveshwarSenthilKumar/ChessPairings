            ...
            if round_number == 1:
                ...
                if len(unpaired) % 2 != 0:
                    ...
                    bye_player = min(
                        unpaired,
                        key=lambda p: (
                            sum(1 for r in self.get_player_pairing_history(tournament_id, p['id']) 
                                if r.get('black_player_id') is None),
                            p.get('rating', 0)
                        )
                    )
                    ...
                    print(f"Assigned bye to {bye_player.get('name', 'Unknown')}")
                
                # Pair remaining unpaired players (if any)
